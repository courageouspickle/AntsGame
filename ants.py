    ...
    def nearest_bee(self, beehive):
        ...
        def cycle(place):
            ...
            if distance < self.min_range:
                distance = distance + 1
                return cycle(place.entrance)
            if distance > self.max_range:
                return None
            if place.entrance:  #if a place has an entrace, then it's not the hive
                if place.bees:
                    return random_or_none(place.bees)
                else:
                    distance = distance + 1
                    return cycle(place.entrance)
        return cycle(self.place)
...
class BodyguardAnt(ContainerAnt):
    ...
    # BEGIN Problem 9
    def __init__(self, *args, **kwargs):
        Ant.__init__(self, armor = 2, *args, **kwargs)
        self.contained_ant = None
    implemented = True   # Change to True to view in the GUI
    # END Problem 9
...
# END Problem 13
    ...
    def action(self, gamestate):
        ...
        if self.true_queen:
            ...
            while p.exit:
                if p.ant:
                    if p.ant not in self.buffed_ants:
                        p.ant.damage = p.ant.damage * 2
                        self.buffed_ants.append(p.ant)
                    if p.ant.contained_ant:
                        if p.ant.contained_ant not in self.buffed_ants:
                            p.ant.contained_ant.damage = p.ant.contained_ant.damage * 2
                            self.buffed_ants.append(p.ant.contained_ant)
                p = p.exit
            ThrowerAnt.action(self, gamestate)
        else:
            Insect.reduce_armor(self, self.armor)  #using Insect.reduce_armor instead of self.reduce armor prevents ending the game
        # END Problem 13
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 13
        if self.true_queen and self.armor - amount <= 0:
            bees_win()
        else:
            Insect.reduce_armor(self, amount)
        # END Problem 13
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # BEGIN Problem 7
        return self.place.ant.blocks_path if (self.place.ant is not None) else False
        # END Problem 7
...
def make_slow(action, bee, duration = 3):
    ...
    def slowed_action(gamestate):
        nonlocal slow_timer
        if slow_timer and gamestate.time % 2 == 0:
            action(gamestate)
            slow_timer = slow_timer -1
        elif slow_timer and gamestate.time % 2 == 1:
            slow_timer = slow_timer -1
        else:
            bee.action = action
